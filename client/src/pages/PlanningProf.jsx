...
const PlanningProf = () => {
    ...
    useEffect(() => {
        ...
        const fetchPlanning = async () => {
            try {
                ...
                // Correction de la route pour correspondre au backend (/api/planning)
        ...
        fetchPlanning();
    }, [prof?.id_unique]); // Dépendance stable pour éviter les boucles
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 w-full space-y-8 pb-12">
            ...
            {events.length > 0 ? (
                <div className="bg-white p-6 sm:p-8 rounded-[3.5rem] shadow-sm border border-slate-100 overflow-x-auto w-full">
                ...
                <div className="h-96 flex flex-col items-center justify-center bg-slate-50 rounded-[3.5rem] border-2 border-dashed border-slate-200 text-center p-12">
                    ...
                    <p className="text-sm text-slate-400 font-medium max-w-xs">
                        L'identifiant <span className="text-violet-600 font-bold">{prof?.id_unique}</span> n'apparaît dans aucun créneau.
                    </p>
                </div>
            )}
        </div>
